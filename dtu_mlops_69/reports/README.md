...
* [x - vores model er lort og har brug for at trænes meget bedre] Add a model to `model.py` and a training procedure to `train.py` and get that running (M6)
...
--- 
We used the bucket service for two purposes. One for storing our data, which can be seen ![here](figures/bucket_data.png), and the other for storing our model artifacts, which can be seen [here](reports/figures/bucket_model.png). The bucket service allowed us to store and access data and model artifacts in a scalable and secure manner.---
---
