...
Additionally, setting up workflows for distributed training and monitoring highlighted the challenges of creating scalable solutions while maintaining simplicity. Despite these hurdles, the focus on infrastructure provided valuable insights into operationalizing machine learning workflows, which are essential for real-world applications where data evolves continuously. In our case the data was from a static data set, which were not continuously updated. Ideally it would have been interesting to deploy these CI/CD pipelines with a dynamic expanding data set.
