...
class Model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        sent_id, mask, targets = batch  # Unpack the batch into input_ids, attention_mask, and target
        ...
        if batch_idx % 100 == 0:
            print(f"Step {batch_idx}: train_loss = {loss.item()}, train_acc = {acc.item()}")
        return loss
    ...
    def train_dataloader(self, batch_size: int) -> torch.utils.data.DataLoader:
        ...
        return torch.utils.data.DataLoader(train_set, batch_size = batch_size, num_workers = 4, pin_memory=True)
    ...
    def val_dataloader(self, batch_size: int) -> torch.utils.data.DataLoader:
        ...
        return torch.utils.data.DataLoader(val_set, batch_size = batch_size, num_workers = 4, pin_memory=True)
...
def train(lr: float = 0.001, batch_size: int = 32, epochs: int = 5) -> None:
    ...
    trainer = Trainer(accelerator="auto", devices=1,log_every_n_steps = 10,
                    max_epochs=epochs, limit_train_batches=0.2, 
                    callbacks=[early_stopping_callback, checkpoint_callback], 
                    logger=pl.loggers.WandbLogger(project="dtu_mlops"))

    # Train the model
    trainer.fit(model, train_dataloaders=model.train_dataloader(batch_size), val_dataloaders=model.val_dataloader(batch_size))
